...
값을 보여주어 `NON-REPEATABLE READ`뿐만 아니라 `PHANTOM READ` 현상도 방지한다.

하지만 InnoDB를 사용하고 있다 하더라도 데이터가 보였다 안 보였다 하는 현상인 `PHANTOM READ`가 발생할 수도 있다.
그것은 바로 `SELECT`가 아니라 `SELECT ... FOR UPDATE`를 사용하는 경우이다.
`SELECT ... FOR UPDATE`는 새로운 레코드가 아닌 기존에 존재하는 레코드에 대한 변경을 제어하는 목적으로 사용된다.
따라서 새로운 레코드가 삽입되는 것에 대해서 잠금을 거는 것은 아니다. 또한 `SELECT` 쿼리는 트랜잭션 시작 시점의 스냅샷을 기반으로 언두 로그를 참조하여
트랜잭션 시작 시점의 데이터만 조회하는 것으로 새로운 데이터 삽입 혹은 변경에 대한 영향을 받지 않는데, `SELECT ... FOR UPDATE` 쿼리는 
트랜잭션 스냅샷과 상관없이 최신의 데이터를 조회하기 때문에 새로운 데이터가 추가되는 `PHANTOM READ` 현상이 발생할 수 있는 것이다.

> 그렇다고 해서 `SELECT ... FOR UPDATE`를 사용한다고 해서 항상 `PHANTOM READ`가 발생하는 것은 아니다. 갭 락과 넥스트 키 락을 활용하여 인덱스 락을
> 걸게 되면 `SELECT ... FOR UPDATE`에 맞는 인덱스에 쓰기 잠금이 걸리기 때문에 해당 인덱스 범위에 맞는 데이터를 삽입할 수 없게 되어 
> `PHANTOM READ`를 방지할 수 있다. 
> 
> 이것이 InnoDB가 갭 락과 넥스트 키 락 덕분에 `REPEATABLE READ` 격리 수준에서도 `PHANTOM READ`를 방지한다고 하는 이유이다.
